...
- `PublicationProject 1.3`、21 类语义样式、确定性继承、属性级锁定与开本版心规格已经进入同一出版契约；1.0—1.2 项目迁移、诗词/译文独立性、主题切换保锁及目标覆盖输入校验均有自动化测试。
- `pnpm check` 通过：7 个工作包类型检查、7 个测试文件共 42 项单元/集成测试和生产构建成功；最终构建上的真实 Electron E2E 4/4 通过。
- Electron 回归实际修改诗词正文字号并核验计算样式，确认篇章标题未受牵连；锁定诗词行距后切换主题，锁定值保持且未锁定标题对齐随主题更新。深色窄屏、115% 缩放与减弱动态效果下出版控件无横向溢出。
