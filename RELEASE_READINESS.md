...
- `pnpm typecheck`、39 项单元/集成测试、生产构建和真实 Electron E2E 4/4 已通过；实际断言样式表挂载、素笺标题居中、当代标题左对齐、导出页集中修正、PDF 与 EPUB 成品有效。
- 正式 SQLite 仅使用既有在线备份的二次临时副本；在其中构造同图多篇复用后，重复素材预检归并测试 1/1 通过，正式库未被写入。
- Developer ID 签名、公证、Gatekeeper、带隔离属性冷启动与 GitHub 预发布状态待 0.3.2 发行阶段完成后补录。
