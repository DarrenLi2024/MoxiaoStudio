...
- 结构化建议协议、本地离线检查器、OpenAI-compatible BYOK 连接器、输入哈希审计与逐条人工决定已进入同一智校契约；模型输出受字段白名单、响应校验和母本基线冲突门禁约束，不能直接覆盖活母本。
- 建议收件箱、批量补录暂存预览、不可变版本查看与恢复、全局命令搜索均已接入真实桌面链路；远端智校默认关闭，启用后仍须明确确认发送范围，密钥只通过系统安全存储持久化。
- `pnpm check` 通过：8 个工作包类型检查、8 个测试文件共 47 项单元/集成测试和生产构建成功；冻结构建上的真实 Electron E2E 5/5 通过，覆盖智校建议人工接受与批量补录预览。
- 生产依赖已知漏洞为零；常见私钥、令牌和 Apple App 专用密码严格模式扫描无命中。宽松扫描中的历史公证 UUID、项目 UUID 与 CSS 文本已逐项确认属于误报。
- 0.5.0 应用公证提交 `a32dd80a-75a1-4640-8d55-91ff1a103ae3`、DMG 公证提交 `308ad573-0b09-4214-b2b0-3ccf381a93f4` 均由 Apple 接受；应用与 DMG 的票据、Developer ID 签名及 Gatekeeper 独立复验通过。
- 最终 DMG 副本添加 `com.apple.quarantine` 后完成 CRC 校验与只读挂载，包内应用票据、深层签名和 Gatekeeper 通过；打包版冷启动及真实 PDF 导出测试 1/1 通过。
- 最终 Apple Silicon DMG 为 `Moxiao-Studio-0.5.0-arm64.dmg`，119,458,158 字节，SHA-256 `c7bb8217a1611092855b720cd7502c5acbf7dc15c510a15d37bb22b1599dd906`；独立清单复核为 `OK`。
- 远端 BYOK 智校未使用用户密钥执行真实外部服务调用；本次发行验证覆盖本地规则提供方、结构化响应校验、安全配置边界及人工接受链路，不把未配置的外部模型服务写成已通过。
- 发行验收提交 `ac4c66b` 已推送到 `codex/bootstrap`，注释标签 `v0.5.0` 指向该提交；GitHub 预发布已创建并核验为非草稿、预发布状态。
- GitHub DMG 资产为 119,458,158 字节，服务端 SHA-256 `c7bb8217a1611092855b720cd7502c5acbf7dc15c510a15d37bb22b1599dd906` 与本地一致；同时发布 96 字节独立校验清单。
