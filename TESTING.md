...
MVP2 回归另以不匹配题名/体裁构造本地智校建议，断言接受前母本不变、接受后 SQLite 持久化；批量文本必须先显示“尚未写入母本”的解析暂存区，再由用户确认入库。版本入口须能列出不可变快照。深色窄屏下智校三栏不得横向溢出。
...
- 母本到交换包再回灌的语义往返不变性。
- AI 建议字段白名单、远端响应结构、母本基线冲突、接受/拒绝状态和运行审计。
