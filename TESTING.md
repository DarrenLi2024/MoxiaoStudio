...
同一套端到端测试还会走完出版中心六步流程，验证体裁多选、动态顺序、顺序固化、意境编排候选、前置页确认、样式实时渲染、单篇随文锚点插图和文字水印，导出真实 A5 Tagged PDF，并复核文件签名、页数和结束标记。
样式实时渲染必须断言预览子文档存在已挂载样式表，并分别核对居中与左对齐主题的实际计算样式；只断言 HTML 中包含 CSS 不视为通过。插图回归先保留缺失替代文字和未知权属，再在导出页集中生成草稿、确认权属并验证门禁解除。
