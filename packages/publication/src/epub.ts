...
export function renderEpub(document: PublicationDocument, project: PublicationProject, assets: readonly PublicationAssetDeclaration[] = []): Uint8Array {
  ...
  const css = `${fontFaces}:root{--accent:${project.theme.accentColor}}body{font-family:${project.theme.bodyFont};font-size:1rem;line-height:${project.theme.lineHeight};color:#20241f}h1,h2,h3{font-family:${project.theme.headingFont};color:var(--accent)}h1{text-align:${project.theme.titleStyle === "left-modern" ? "left" : "center"}}p{margin-block:0 ${project.theme.density === "compact" ? ".72rem" : project.theme.density === "relaxed" ? "1.18rem" : ".92rem"}}.verse{text-align:center;line-height:2}.verse p{margin:0}aside{border-left:.12rem solid var(--accent);padding:.6rem 1rem}.block-translation{${project.theme.translationStyle === "panel" ? "padding:1rem;background:#f3f5f1" : project.theme.translationStyle === "rule" ? "padding-left:1rem;border-left:.08rem solid var(--accent)" : ""}}.block-appreciation{${project.theme.appreciationStyle === "panel" ? "padding:1rem;background:#f7f5ef" : project.theme.appreciationStyle === "rule" ? "padding-top:1rem;border-top:.08rem solid #d4d7d1" : ""}}.book-toc ol{padding:0;list-style:none}.book-toc li{display:flex;gap:1rem;padding:.45rem 0;border-bottom:1px dotted #c8cdc6}.book-toc li span{color:#747b74}figure{text-align:center;break-inside:avoid}figure.align-left{text-align:left}figure.align-right{text-align:right}figure.image-small img{max-width:35%}figure.image-medium img{max-width:62%}figure.image-wide img{max-width:100%}figure.image-full-page{break-before:page;break-after:page}img{max-width:100%;height:auto;object-fit:contain}figcaption{font-size:.82rem;color:#70776f}`;
  ...
  const nav = `<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" lang="${xml(document.language)}"><head><title>目录</title></head><body><nav epub:type="toc"><h1>目录</h1><ol>${bodyChapters.map((chapter) => `<li><a href="${chapter.file}">${xml(chapter.title)}</a></li>`).join("")}</ol></nav><nav epub:type="landmarks" hidden="hidden"><ol>${landmarks}</ol></nav></body></html>`;
